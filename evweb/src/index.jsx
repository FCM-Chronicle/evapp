            ...
            <div className="flex-1 px-4 py-5 flex flex-col">
                <Section title="1. AI 모델 설정 (NVIDIA, Groq, OpenAI)">
                    ...
                    <input
                        ...
                        style={{ ...mono, color: C.accent, fontSize: 12 * scale, padding: `${10 * scale}px`, border: `1px solid ${C.panelBorder}` }}
                    />
                    <span style={{ ...mono, color: C.slate, fontSize: 10 * scale }}>Model Name</span>
                    <input
...
function HistoryScreen({ onBack, archives, onSelect }) {
    ...
    return (
        <div className="flex flex-col h-full">
            <StatusBar showBack onBack={onBack} title="이전 대화" />
            <div className="flex-1 overflow-y-auto px-5 py-4">
                {archives === null && (
                    <div className="flex items-center justify-center h-full">
                        <span style={{ ...mono, color: C.slate, fontSize: 12 }}>불러오는 중...</span>
                    </div>
                )}
                {archives !== null && archives.length === 0 && (
                    <div className="flex items-center justify-center h-full">
                        <span style={{ ...mono, color: C.slate, fontSize: 12 }}>보관된 이전 대화가 없습니다</span>
                    </div>
                )}
                {archives !== null && archives.length > 0 && (
                    <div className="flex flex-col gap-2">
                        {archives.map((a) => (
                            <button
                                ...
                                style={{ border: `1px solid ${C.panelBorder}`, background: C.panel }}
                            >
                                <div className="flex items-center gap-2">
                                    <History size={14} color={C.slate} />
                                    <span style={{ ...mono, color: C.text, fontSize: 12 }}>{formatDate(a.date)}</span>
                                </div>
